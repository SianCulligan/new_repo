...
X 1. Clone this repository to your machines
X 1. Create a postgres database called `code_301_assessment`
X 1. Copy `.env-sample` to `.env`, making any changes appropriate for your system
X 1. From within the folder you cloned into, run these commands
...
1. Convert to jQuery
   ...
   XXXX 1. Load jQuery from a CDN
   1. Replace as much of the existing vanilla JS with jQuery best practices

XXXX 1. Convert the existing CSS to SMACSS organiztion

XXXX 1. Add some creative styling to the application, adding styles to the appropriate SMACSS files
   XXXX 1. Use flexbox or grid to lay out the character cards in a responsive grid.
   XXXX 1. Incorporate a color theme
   XXXXX 1. Fix the footer to the bottom of the viewport
   XXXXX 1. Add at least one more meaningful improvement to the design

XXX 1. Add a button to the bottom of the page labeled "Get More Characters" that will dynamically load the next bunch of characters from the Star Wars API and render them on the page alongside the others:
   1. Add Handlebars to your page
      XXXX 1. Load Handlebars from a CDN
      xxxx 1. Create a Handlebars template for a single character `div`
   ...
   XXXX 1. Create a new `get` route called `/characters` on the server, that will return a single page of character information as JSON:
